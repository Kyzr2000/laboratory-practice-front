...
import { DELETE_EXPERIENCE } from '../graphql/experienceGql';
...
const ExperienceManagementTable = ({
  experience,
  loading,
  error,
  refreshData,
  ...
  const columns: ColumnsType<Experience> = [
    ...
    },
    {
      ...
      key: 'username',
      width: 90,
      render: (_, record) => <span>{record.user?.username || '未知用户'}</span>,
      align: 'center',
    },

    {
      ...
      width: 200,
      render: (_, record) => (
        <span className="experience-address">{formatAddress(record)}</span>
      ),
      align: 'center',
    },
    {
...
export default ExperienceManagementTable;
